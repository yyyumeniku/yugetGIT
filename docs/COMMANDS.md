...
1. **`/backup save [-m "msg"]`**
   ...
   *   Stages changed chunks to `staging/<dimension>/region/r.x.z/c.x.z.nbt` and commits only `staging` + `meta`.
...
4. `CommitBuilder` stages `staging` and `meta`, commits, then runs `git gc --auto` and `git repack -a -d`.
